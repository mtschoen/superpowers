...
On Windows the shell holds a lock on its own cwd, so removal from inside
the worktree fails with a permission error (`rm -rf` reports "Device or
resource busy") — another reason the directory change in Step 5 must
