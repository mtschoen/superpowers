...
output=$(run_claude "In the subagent-driven-development skill, what comes first: plan compliance review or code quality review? Answer using exactly this structure:
First: <review type>
Second: <review type>" "$CLAUDE_PROMPT_TIMEOUT")

if assert_order "$output" "First:.*plan.*compliance" "Second:.*code.*quality" "Plan compliance before code quality"; then
    : # pass
else
    exit 1
fi
...
output=$(run_claude "What is the plan compliance reviewer's attitude toward the implementer's report in subagent-driven-development?" "$CLAUDE_PROMPT_TIMEOUT")
