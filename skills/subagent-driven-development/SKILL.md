...
Execute plan by dispatching a fresh implementer subagent per task, a task review (plan compliance + code quality) after each, and a broad whole-branch review at the end.

**Why subagents:** You delegate tasks to specialized agents with isolated context. By precisely crafting their instructions and context, you ensure they stay focused and succeed at their task. They should never inherit your session's context or history — you construct exactly what they need. This also preserves your own context for coordination work.

**Core principle:** Fresh subagent per task + task review (plan + quality) + broad final review = high quality, fast iteration
...
- Review after each task (plan compliance + code quality), broad review at the end
...
digraph process {
    ...
    subgraph cluster_per_task {
        ...
        "Plan ✅ and quality approved?" [shape=diamond];
    ...
    "Generate review package, dispatch task reviewer (./task-reviewer-prompt.md)" -> "Plan ✅ and quality approved?";
    "Plan ✅ and quality approved?" -> "Append completion to ledger, mark todo complete" [label="yes"];
    "Plan ✅ and quality approved?" -> "Finding conflicts with plan text?" [label="no"];
...
- Never dispatch multiple implementation subagents in parallel **without
  worktree isolation** (conflicts). Parallel dispatch is allowed only when
  each implementer runs in its own isolated worktree on independent tasks —
  see Parallel Dispatch below.
...
report missing either verdict — plan compliance AND task quality are both
...
- The global-constraints block you hand the reviewer is its attention
  ...
  Constraints section: exact values, exact formats, and the
  stated relationships between components ("same layout as X", "matches
  Y"). The reviewer's template already carries the process rules (YAGNI,
  test hygiene, review method) — the constraints block is for what THIS
  project's plan demands.
...
lacks. If you confirm an item is a real gap, treat it as a failed plan
...
The loop triggers when the review reports plan ❌, any Critical or Important
...
The task loop above is serial by default. When the plan's tasks are
...
**The loop still runs per task.** Each parallel task keeps its own review,
fix rounds, and ledger lines — parallelism changes when implementers are
dispatched, not what a task must pass to be complete. Record BASE per task
before its dispatch, and append that task's ledger lines as it lands.
...
| Excuse | Reality |
|--------|---------|
| "Close enough on plan compliance" | Reviewer found plan gaps = not done. Fix or hit the cap and adjudicate — those are the only exits. |
...
Task reviewer: Plan ✅ - all requirements met, nothing extra.
